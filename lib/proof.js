 ...
 *
 * Read straight off the daily roster snapshot already in memory rather than re-deriving it, so
...
export function record({ key, coin, side, address, pFollow, pBase, followWon, push }) {
  ...
  return appendJsonl(JOURNAL, {
    ...
    p: +pFollow.toFixed(4), ...(Number.isFinite(pBase) ? { pb: +pBase.toFixed(4) } : {}), won: !!followWon, push: !!push,
    ho: isHoldout(key),
  });
}
 ...
 */
/**
 * Brier score, with two references.
 *
 * `reference` guesses this set's OWN win rate on every hand. No forecaster could have done that: the
 * number only exists once the set has resolved, so it is an oracle, and a model can lose to it while
 * still being useful. `priorReference` is the fair comparison when a prior is given: always guessing
 * the base rate the odds were trained on, which is what a forecaster who ignored every Nansen feature
 * would actually have said. Both are published; the page leads with the fair one and names the oracle.
 */
export function brier(rows, prior = null) {
  const r = scored(rows);
  if (!r.length) return null;
  const base = r.filter((x) => x.won).length / r.length;
  const ref = (q) => r.reduce((a, x) => a + (q - (x.won ? 1 : 0)) ** 2, 0) / r.length;
  const ok = Number.isFinite(prior) && prior > 0 && prior < 1;
  return {
    n: r.length,
    ...
    reference: ref(base),
    prior: ok ? prior : null,
    priorReference: ok ? ref(prior) : null,
  };
}
 ...
 *
 * `unknown` is reported rather than hidden: a wallet the daily roster has not assessed sits in ALL
...
export function report(samples = [], botRows = []) {
  ...
  return {
    ...
    forward: {
      ...
      reliability: reliability(fwd), brier: brier(fwd, oddsEngine.getModel().baseRate), strategies: strategies(fwd),
      ...
      clean: (() => {
        ...
        return { n: c.length, scored: scored(c).length, since: c.length ? c[0].at : null, every: HOLDOUT_EVERY,
          reliability: reliability(c), brier: brier(c, oddsEngine.getModel().baseRate), strategies: strategies(c), populations: populations(c) };
      })(),
    },
