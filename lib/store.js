...
/**
 * appendJsonl with a size cap, for logs that grow with every scan rather than with every alert. Past
 * `maxBytes` the file is rotated to `<name>.1.jsonl` (replacing any older one), so the log never
 * holds more than about twice the cap on disk and the newest rows are always in `<name>.jsonl`.
 */
export function appendJsonlCapped(name, obj, maxBytes) {
  const f = path.join(DIR, name + '.jsonl');
  try {
    if (maxBytes > 0 && fs.existsSync(f) && fs.statSync(f).size >= maxBytes) fs.renameSync(f, path.join(DIR, name + '.1.jsonl'));
  } catch (e) { console.error(`[store] could not rotate ${name}.jsonl: ${e.message}`); }
  return appendJsonl(name, obj);
}
