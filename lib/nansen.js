...
// Every endpoint this app calls, and what Nansen charges for it. Four more used to sit here
// (perp-leaderboard, profiler/perp-positions, profiler/perp-trades, tgm/perp-trades) with prices but
// no call site anywhere in the code. A priced endpoint nobody calls is not breadth, it is a list of
// things that were considered, so they are gone. These four are the whole Nansen surface, plus the
// two Agent modes billed in lib/agent.js.
const COST = {
  'smart-money/perp-trades': 5, 'perp-screener': 1,
  'profiler/perp-pnl-summary': 1, 'tgm/perp-positions': 5,
};
...
export const getUsageDetail = () => ({
  ...
  byEndpoint: Object.fromEntries(Object.entries(usage.byEndpoint).map(([k, v]) =>
    [k, { calls: v.calls, credits: v.credits, perCall: COST[k] ?? 1 }])),
});
...
export async function smartMoneyOpens({ lookbackHours = 168, minValueUsd = 25000, pages = 2, perPage = 500 } = {}) {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    const batch = r.data || [];
    out.push(...batch);
    if (!wantsNextPage(r, batch.length, perPage)) break;
  }
