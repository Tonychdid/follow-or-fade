...
// BETA_ENDS (YYYY-MM-DD, UTC) sets the countdown shown on the site.
const BETA_ENDS = process.env.BETA_ENDS || '2026-10-11';
const PLANS = ['free', 'basic', 'premium'];
const list = store.load('waitlist', []);

export function beta() {
  const ends = Date.parse(BETA_ENDS + 'T23:59:59Z');
  const daysLeft = Number.isFinite(ends) ? Math.max(0, Math.ceil((ends - Date.now()) / 864e5)) : null;
  return { ends: BETA_ENDS, daysLeft, waitlist: list.length };
}
...
export function join({ email, plan, player }) {
  ...
  return { ok: true, already: !!existing, position: list.findIndex((x) => x.email === e) + 1, ...beta() };
}
