...
export async function scan({ force = false } = {}) {
  ...
  scanning = true;
  try {
    const trades = await nansen.smartMoneyOpens({ lookbackHours: LOOKBACK_H, pages: 1, perPage: 300, minValueUsd: cfg.minSizeUsd });
...
function readLines(a) {
  ...
  const tells = (r.reasons || []);
  if (tells.length) {
    out.push('The dealer\u2019s tells \u2014 \u{1F7E3} favors FOLLOW, \u{1F7E0} favors FADE:');
    for (const x of tells) out.push(`${x.good ? '\u{1F7E3}' : '\u{1F7E0}'} ${x.text}`);
  }
  if (r.positioning) {
