...
async function watchAlert(a) {
  ...
  watches.push({ alertId: a.id, key: a.key, address: a.address, coin: a.coin, side: a.side, trader: a.trader, entryPrice: a.entryPrice,
    openedAt: a.openedAt, t: Date.now(), startSz: sz, peak: Math.abs(sz), lastNotifiedSz: Math.abs(sz), trims: [], done: false });
...
async function sendExitTelegram(a) {
  ...
  const text = a.kind === 'add' ? [
    ...
    `Bet on it: ${site}/?view=live&trade=${encodeURIComponent(a.key)}`,
  ] : a.kind === 'exit' ? [
