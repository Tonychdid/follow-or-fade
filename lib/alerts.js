...
function recordBlock(r) {
  ...
  // A wallet whose two windows are the same wallet is a NEW wallet, and the table cannot say so on
  // its own: a reader sees "30D - 648 closes - +$124.2K" and takes it for a month of track record
  // when the whole thing happened this week. The numbers are right and the impression is wrong, so
  // the message says it outright. Detected by the 7-day column having caught up with the month
  // rather than by asking for a first-trade date, which would cost another call per alert.
  ...
  return ['', '<pre>' + grid + '</pre>',
    ...
    ...(allInLastWeek
      ? [`\u{26A0}\u{FE0F} <b>New wallet.</b> Both columns are the same because this wallet's entire record `
        + `is inside the last 7 days \u2014 there is no 30-day history behind it.`]
      : [])];
}
