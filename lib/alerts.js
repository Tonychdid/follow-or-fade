...
function readLines(a) {
  ...
  if (r.moveSinceEntry != null) {
    ...
    const age = opened ? ` · they opened ${hrsTxt(Date.now() - opened)} ago` : '';
    const where = `${a.coin} ${raw >= 0 ? 'up' : 'down'} ${Math.abs(raw).toFixed(2)}% from the whale's entry`;
    out.push(Math.abs(m) < 0.3 ? `Entry now: within ${Math.abs(m).toFixed(2)}% of the whale's own entry price (${where})${age}`
      : m > 0 ? `Entry now: ${where}, so this ${a.side.toLowerCase()} is already ${m.toFixed(2)}% up${age} — entering now is ${m.toFixed(2)}% worse than the whale's own fill`
      : `Entry now: ${where}, so this ${a.side.toLowerCase()} is ${Math.abs(m).toFixed(2)}% underwater${age} — entering now is ${Math.abs(m).toFixed(2)}% better than the whale's own fill`);
  }
