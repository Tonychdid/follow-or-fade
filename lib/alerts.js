...
const PIN_MAX = Math.max(1, Number(process.env.ALERT_PIN_MAX || 8));
game.setAlertLookup((key) => alerts.find((a) => !a.kind && a.key === key) || null);
game.setExtraLive(() => {
  const keys = [];
  const add = (k) => { if (k && !keys.includes(k)) keys.push(k); };
  [...watches].filter((w) => !w.done).sort((a, b) => b.t - a.t).forEach((w) => add(w.key));
...
async function watchAlert(a, knownSz) {
  ...
  const w = { alertId: a.id, key: a.key, address: a.address, coin: a.coin, side: a.side, trader: a.trader, entryPrice: a.entryPrice,
    openedAt: a.openedAt, valueUsd: a.valueUsd, raw: a.raw || null, t: Date.now(), startSz: null, peak: 0, lastNotifiedSz: 0, trims: [], done: false, pending: true, misses: 0 };
  watches.push(w);
