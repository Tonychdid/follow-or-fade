...
function pushExitAlert(w, extra) {
  ...
  const a = { id: crypto.randomUUID(), t: Date.now(), key: w.key, coin: w.coin, side: w.side, trader: w.trader, address: w.address,
    ...
    scalper: !!parent?.scalper, tradesPerDay: parent?.tradesPerDay ?? null,
...
function whoLine(a) {
  ...
  const pace = a.scalper && a.tradesPerDay ? ` · SCALPER (~${Math.round(a.tradesPerDay)} closes/day, so expect a short hold)` : '';
  const who = (a.specialist ? `${a.coin} specialist` : `Grade ${g || '?'}`) + pace;
...
function exitText(a, note = null) {
  ...
  const spec = (a.specialist ? ' · SPECIALIST' : '') + (a.scalper ? ' · SCALPER' : '');
...
function alertText(a, intel = [], note = null) {
  const r = a.record, d30 = r?.d30 || {}, d7 = r?.d7;
  return [
    `🐋 WHALE ALERT${a.specialist ? ' · SPECIALIST' : ''}${a.scalper ? ' · SCALPER' : ''} · Grade ${r?.trust?.grade || '?'} (${r?.trust?.score ?? '–'}/100)`,
