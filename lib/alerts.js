...
const DEFAULTS = {
  enabled: true, minGrade: 'A', minWinRate: 0.55, minClosed: 20, minSizeUsd: 50000, requireProfit30d: true,
...
async function sendTelegram(a) {
  ...
  const text = [
    ...
    ``,
    `30D: ${d30.pnl >= 0 ? '+' : ''}${money(d30.pnl)} realized · ${Math.round((d30.winRate || 0) * 100)}% win rate · ${d30.closed} closes`,
    d7 && d7.closed ? `7D: ${d7.pnl >= 0 ? '+' : ''}${money(d7.pnl)} realized · ${Math.round((d7.winRate || 0) * 100)}% win rate` : `7D: no closed trades`,
    ``,
