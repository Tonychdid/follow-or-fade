...
export function captionFor(a, { owner = false, recheck = null } = {}) {
  ...
  if (!a.kind) {
    ...
    lines = [
      ...
      ...deskLine(a, owner),
      `${who} opened ${pos} · ${money(a.valueUsd)} @ <code>${esc(priceTxt(a.entryPrice))}</code>${opened ? ` · ${hrsTxt(Date.now() - opened)} ago` : ''}`,
      ...lateLine(a),
...
function extraSections(a, rc) {
  ...
  } else if (a.read && (a.read.pFollow != null || a.read.moveSinceEntry != null)) {
    out.push({ header: ['NOW', '', ''], rows: [
      ...(a.read.pFollow != null ? [['Whale-trade score', pc(a.read.pFollow)]] : []),
      ...(a.read.moveSinceEntry != null ? [['Whale P&L from fill', sg(a.read.moveSinceEntry)]] : []),
    ] });
  }
...
async function recheck(q) {
  ...
  const rc = { hhmm: nowIso.slice(11, 16), ago, baseMid, mid: item.mid, since, whaleNow,
    whaleThen: entryRef && baseMid ? dir * (baseMid - entryRef) / entryRef : null,
