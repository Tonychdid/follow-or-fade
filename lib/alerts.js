...
/** The numbers that used to sit in the caption, drawn in the picture instead. */
function extraSections(a, rc) {
  const out = [];
  const sg = (v, d = 2) => (v == null || !Number.isFinite(v) ? DASH : `${v >= 0 ? '+' : '-'}${Math.abs(v * 100).toFixed(d)}%`);
  const pc = (v) => (v == null ? DASH : `${Math.round(v * 100)}%`);
  if (rc) {
    out.push({ header: ['RE-CHECK', 'ALERT', 'NOW'], rows: [
      ['Price', priceTxt(rc.baseMid), priceTxt(rc.mid)],
      ...(rc.avgThen || rc.avgNow ? [['Whale avg entry', rc.avgThen ? priceTxt(rc.avgThen) : DASH, rc.avgNow ? priceTxt(rc.avgNow) : DASH]] : []),
      ...(rc.sizeThen || rc.sizeNow ? [['Size', rc.sizeThen ? money(rc.sizeThen) : DASH, rc.sizeNow ? money(rc.sizeNow) : DASH]] : []),
      ...(rc.pThen != null || rc.pNow != null ? [['Whale-trade score', pc(rc.pThen), pc(rc.pNow)]] : []),
      ['Whale P&L', sg(rc.whaleThen), sg(rc.whaleNow)],
      ['Since the alert', sg(rc.since)],
    ] });
  } else if (!a.kind) {
    // The whale's own average entry on Hyperliquid (their first fill when the average is unknown).
    const avg = +a.avgEntry || +a.entryPrice;
    const mid = +a.read?.mid;
    const dir = a.side === 'Short' ? -1 : 1;
    const pnl = avg && mid ? dir * (mid - avg) / avg : a.read?.moveSinceEntry ?? null;
    out.push({ header: ['POSITION', '', ''], rows: [
      ...(Number.isFinite(a.valueUsd) ? [['Size', money(a.valueUsd)]] : []),
      ...(avg ? [['Whale avg entry', priceTxt(avg)]] : []),
      ...(mid ? [['Price now', priceTxt(mid)]] : []),
      ...(pnl != null ? [['Whale P&L', sg(pnl)]] : []),
      ...(a.read?.pFollow != null ? [['Whale-trade score', pc(a.read.pFollow)]] : []),
    ] });
  }
  const ex = rc ? rc.exits : a.kind === 'exit' ? null : a.exits;
  if (ex && !(rc && rc.gone)) {
    ...
    out.push({ header: ["WHALE'S EXITS", 'PRICE', 'FROM NOW'], rows });
  }
  return out;
}
...
export function photoFor(a, text, opts = {}) {
  ...
  const png = IMG.render({ title: `${String(a.side).toUpperCase()} ${String(a.coin).split(':').pop()}`, subtitle: who, badge,
    sections: [...IMG.sectionsOf(grids), ...extraSections(a, opts.recheck)] });
...
async function recheck(q) {
  ...
  const rc = { hhmm: nowIso.slice(11, 16), ago, baseMid, mid: item.mid, since, whaleNow,
    ...
    gone: !!item.gone, trimmed: item.trimmed || 0,
