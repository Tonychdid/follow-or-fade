...
// altcoins that delay cost about 0.2% per copied trade. The feed cache now expires just before the
// next scan, and the scan runs every 2 minutes (SCAN_MINUTES): 5 credits a scan, about 3,600 a day.
const SCAN_FLOOR = Math.max(1, Number(process.env.SCAN_MIN_MINUTES) || 2);
const SCAN_TARGET = Math.max(SCAN_FLOOR, Number(process.env.SCAN_MINUTES) || 2);
// One-time move of an existing deployment (saved at 10) to the new interval; the owner can still
// change it afterwards from the settings panel.
if (!cfg.scanV2) { cfg.intervalMin = SCAN_TARGET; cfg.scanV2 = true; save('alertcfg', cfg); }
...
const pubTrust = (t) => (t ? { grade: t.grade, score: t.score, label: t.label, marketMaker: !!t.marketMaker, tooFast: !!t.tooFast, scalper: !!t.scalper } : t);
...
export function assess(rec, coin, { onBoard = false } = {}) {
  ...
  if (trust?.marketMaker) return no('market-maker pattern: wins almost every close, which is spread, not direction');
  if (trust?.tooFast) return no('too fast to copy: short holds and near-certain wins, the edge is gone before a follower is in');
...
export async function scan({ force = false } = {}) {
  ...
  try {
    ...
    for (const t of fresh) {
      ...
      if (a.side === 'Short' && !t._demo && !nansen.isDemo()) {
        const cover = await hl.spotCover(a.address, a.coin, Number.isFinite(live.valueUsd) && live.valueUsd > 0 ? live.valueUsd : +t.value_usd).catch(() => null);
      ...
      if (!t._demo && !nansen.isDemo()) a.exits = await readExits(a.address, a.coin, a.side).catch(() => null);
