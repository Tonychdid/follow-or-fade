...
export async function scan({ force = false } = {}) {
  ...
  try {
    ...
    for (const t of fresh) {
      ...
      if (item) a.read = { pFollow: item.pFollow, odds: item.odds, reasons: (item.reasons || []).slice(0, 3), positioning: item.positioning, moveSinceEntry: item.moveSinceEntry };
...
async function sendTelegram(a) {
  ...
  const text = [
    ...
    `Full whale card (track record, tells, Smart Money positioning): ${(process.env.PUBLIC_URL || `http://localhost:${process.env.PORT || 3000}`).replace(/\/$/, '')}/?view=live&trade=${encodeURIComponent(a.key)}`,
  ].join('\n');
