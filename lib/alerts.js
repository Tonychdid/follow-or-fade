...
async function broadcast(payload) {
  ...
  for (const s of [...subs]) {
    ...
    try { await tg(cfg.telegram.token, 'sendMessage', { ...payload, chat_id: s.id }); sent++; }
...
export async function scan({ force = false } = {}) {
  ...
  try {
    ...
    for (const t of fresh) {
      ...
      const kind = qualifies(rec, t.token_symbol) || (onBoard ? 'leaderboard' : null);
      if (!kind) continue;
...
const tagStr = (a) => (a.leaderboard ? ' · LEADERBOARD' : '') + (a.specialist ? ' · SPECIALIST' : '') + (a.early ? ' · EARLY' : '')
...
async function warnStale(w) {
  ...
  if (cfg.telegram.chatId) {
    await tg(cfg.telegram.token, 'sendMessage', { ...staleBody, chat_id: cfg.telegram.chatId })
...
function alertText(a, intel = [], note = null) {
  ...
  return [
    ...
    ...(a.leaderboard ? ['(Top 30 on the whale board by 30-day return, over a real sample of closed trades.)'] : []),
    ``,
...
async function sendTelegram(a) {
  ...
  if (cfg.telegram.chatId) {
    const sent = await tg(cfg.telegram.token, 'sendMessage', { ...body, chat_id: cfg.telegram.chatId });
