...
import { load, save, appendJsonl, appendJsonlCapped, readJsonl } from './store.js';
...
// Alerts that scrolled out of the 200 kept above, for 7 days, so a Refresh on a busy day's older card
// still works (owner, Sep 25: "That alert is too old to re-check"). One line per alert, never
// rewritten; a re-checked alert is appended again and the newest line wins when read back.
const ARCHIVE_DAYS = 7;
const ARCHIVE_MAX = 20000;
const archive = new Map();
for (const a of [...readJsonl('alertarchive.1'), ...readJsonl('alertarchive')]) {
  if (a?.id && Date.now() - (a.t || 0) < ARCHIVE_DAYS * 86400e3) { archive.delete(a.id); archive.set(a.id, a); }
}
function retire(a) {
  if (!a?.id) return;
  archive.delete(a.id); archive.set(a.id, a);
  while (archive.size > ARCHIVE_MAX) archive.delete(archive.keys().next().value);
  appendJsonlCapped('alertarchive', a, 40 * 1024 * 1024);
}
export const _archive = { retire: (a) => retire(a), byId: (id) => alertById(id) };   // tests
/** An alert by id: the recent 200, else the 7-day archive. */
const alertById = (id) => {
  const a = alerts.find((x) => x.id === id) || archive.get(id) || null;
  return a && (alerts.includes(a) || Date.now() - (a.t || 0) < ARCHIVE_DAYS * 86400e3) ? a : null;
};
...
export function assess(rec, coin, { onBoard = false } = {}) {
  ...
  // ---- SCALPER route (owner, Sep 24): a whale that holds for minutes to a few hours gets in at grade B,
  // one step below the usual bar, as long as the record is still real: the 30D return bar, a green
  // last week and the win-rate bar all apply. It feeds the desk's scalpers book; the B1 bot never
  // copies scalpers (BOT ELIGIBLE says PUBLIC ONLY for them).
  if (cfg.allowScalperB !== false && trust.scalper && GRADE_RANK[trust.grade] >= GRADE_RANK.B) {
    if ((d30.roi ?? 0) < (cfg.minRoi30d ?? 0)) return no('SCALPER_ROI', `scalper, but 30D return ${((d30.roi ?? 0) * 100).toFixed(2)}% is under ${((cfg.minRoi30d ?? 0) * 100).toFixed(2)}%`);
    const lw = losingWeek(); if (lw) return { ...lw, gate: 'SCALPER_7D', reason: `scalper, but ${lw.reason}` };
    if ((d30.winRate ?? 0) < cfg.minWinRate) return no('SCALPER_WIN_RATE', `scalper, but win rate ${Math.round((d30.winRate ?? 0) * 100)}% is under ${Math.round(cfg.minWinRate * 100)}%`);
    return { kind: 'scalper', reason: null, gate: null };
  }
...
// Mirrors of two more desk rules, so BOT ELIGIBLE means what B1 will actually consider (owner, Sep 25:
// a $20K PRINTER alert was labelled BOT ELIGIBLE, then B1 skipped it). B1 only takes whales that came
// in through the standard or specialist route (the desk's ALLOWED_ROUTES), and only positions of at
// least the desk's WHALE_MIN_USD.
export const BOT_ROUTES = () => String(process.env.BOT_ROUTES ?? 'standard,specialist').split(',').map((x) => x.trim()).filter(Boolean);
export const BOT_MIN_WHALE_USD = () => Math.max(0, Number(process.env.BOT_MIN_WHALE_USD ?? 25000));
/**
...
export function botEligibility(a) {
  ...
  if (a.admittedVia && !BOT_ROUTES().includes(a.admittedVia)) return [false, `${a.admittedVia} route is public only`];
  if (Number.isFinite(+a.valueUsd) && +a.valueUsd < BOT_MIN_WHALE_USD()) return [false, `whale position ${money(+a.valueUsd)} is under the bot's ${money(BOT_MIN_WHALE_USD())} minimum`];
...
async function considerTrade(t, { key, row, created, lb, hour }) {
  ...
  // Hold time (and so SCALPER) for a whale the daily roster has not scored yet: read it now, or the
  // bot could never be told this alert is eligible. A failed read stays "unknown".
  if (live) await tclass.ensure(t.trader_address).catch(() => null);
  ...
  if (Number.isFinite(pos.valueUsd) && pos.valueUsd > 0) {
    ...
    // The position as the alert found it, for the picture's FIRST column (size in coins, leverage).
    a.pos0 = { sz: Number.isFinite(pos.sz) ? Math.abs(pos.sz) : null, upnl: pos.upnl ?? null, lev: pos.lev ?? null, liqPx: pos.liqPx ?? null };
  }
...
export async function scan({ force = false } = {}) {
  ...
  try {
    ...
    while (alerts.length > 200) retire(alerts.shift());
...
async function stillOpen(a) {
  ...
  return { open: true, checked: true, sz, valueUsd: pos.valueUsd, entryPx: pos.entryPx, upnl: pos.upnl ?? null, lev: pos.lev ?? null, liqPx: pos.liqPx ?? null };
}
...
function pushExitAlert(w, extra) {
  ...
  alerts.push(a); while (alerts.length > 200) retire(alerts.shift());
  save('alerts', alerts);
...
/**
 * The caption of a picture message: a few short lines, never the text message with its grids cut
 * out (owner, Sep 25: "the message is too long"). Every number lives in the picture; the caption only
 * says what happened, who, and - on a re-check - whether the whale is still in. The tag explanations,
 * the Agent brief and the long sentences stay in the text version and on the whale card.
 */
function tagsInline(a) { const t = tagLine(a); return t.length ? ` · ${t[0]}` : ''; }
const heldShort = (rc) => (rc.gone ? '🚪 whale <b>CLOSED</b> it' : rc.trimmed >= 0.1 ? `✂️ whale trimmed ${Math.round(rc.trimmed * 100)}%` : '✅ whale still in');
function recheckLines(rc) {
  if (!rc) return [];
  const out = [`🔄 <b>Re-checked</b> ${esc(rc.hhmm)} UTC · ${esc(rc.ago)} after · ${heldShort(rc)}`];
  if (rc.driftThen && rc.driftNow) out.push(`${(GRADE_RANK[rc.driftNow.grade] ?? 0) > (GRADE_RANK[rc.driftThen.grade] ?? 0) ? '⬆️' : '⬇️'} Grade now <b>${esc(rc.driftNow.grade)}</b>, was ${esc(rc.driftThen.grade)} at the alert`);
  return out;
}
export function captionFor(a, { owner = false, recheck = null } = {}) {
  const who = `<b>${esc(a.trader)}</b>`, pos = `<b>${esc(String(a.side).toUpperCase())} ${esc(a.coin)}</b>`;
  const g = a.record?.trust?.grade, d30 = a.record?.d30;
  const gradeLine = g && d30?.closed ? [`Grade <b>${esc(g)}</b> · 30D ${money$(d30.pnl || 0)} · ${rateOf(d30.winRate)} wins`] : [];
  let lines;
  if (!a.kind) {
    const opened = a.openedAt ? Date.parse(a.openedAt) : null;
    const r = a.record, d7 = r?.d7;
    lines = [
      `\u{1F40B} <b>WHALE ALERT</b> · Grade <b>${esc(r?.trust?.grade || '?')}</b> ${r?.trust?.score ?? DASH}/100${tagsInline(a)}`,
      ...deskLine(a, owner),
      `${who} opened ${pos} · ${money(a.valueUsd)} @ avg <code>${esc(priceTxt(+a.avgEntry || a.entryPrice))}</code>${opened ? ` · ${hrsTxt(Date.now() - opened)} ago` : ''}`,
      ...lateLine(a),
      ...(Number.isFinite(a.tradeUsd) && a.valueUsd > a.tradeUsd * 1.25 ? [`<i>Nansen saw one ${money(a.tradeUsd)} fill; the whale split the entry.</i>`] : []),
      ...(d7?.closed && r?.d30?.closed && d7.closed >= r.d30.closed ? ['⚠️ The whole 30D record is from the last 7 days.'] : []),
    ];
  } else {
    const sz = (n) => (+n).toLocaleString('en-US', { maximumFractionDigits: n >= 100 ? 1 : 4 });
    const base = esc(String(a.coin).split(':').pop());
    lines = a.kind === 'add' ? [`➕ <b>WHALE ADDING</b>${tagsInline(a)}`, `${who} added <b>${sz(a.addedSz)} ${base}</b> to ${pos}: conviction is rising.`]
      : a.kind === 'exit' ? [`\u{1F6AA} <b>WHALE EXIT</b>${tagsInline(a)}`, `${who} closed ${pos}. Following them? Check your own position.`]
      : a.kind === 'trim' ? [`✂️ <b>WHALE TRIM</b>${tagsInline(a)}`, `${who} cut <b>${Math.round(a.trimPct * 100)}%</b> of ${pos}. Following them? Check your position.`]
      : [`\u{1F3AF} <b>WHALE MOVED THEIR EXITS</b>${tagsInline(a)}`, `${who} on ${pos}`];
    lines.push(...gradeLine);
  }
  return [...lines, ...recheckLines(recheck)].join('\n');
}
/**
 * The picture, laid out like the Elite bot's card (owner, Sep 25: "same format as Elite"): the whale's
 * POSITION first, when the alert found it and now, side by side; then the RECORD, 30D beside 7D; then
 * the whale's resting exits. An exit keeps its own short grid.
 */
const sgPct = (v, d = 2) => (v == null || !Number.isFinite(v) ? DASH : `${v >= 0 ? '+' : '-'}${Math.abs(v * 100).toFixed(d)}%`);
const szTxt = (n) => (n == null || !Number.isFinite(+n) ? DASH : (+n).toLocaleString('en-US', { maximumFractionDigits: +n >= 100 ? 1 : 4 }));
function positionSection(a, rc) {
  const dir = a.side === 'Short' ? -1 : 1;
  const pc = (v) => (v == null ? DASH : `${Math.round(v * 100)}%`);
  let first, now;
  if (!a.kind) {
    const avg0 = +(rc?.avgThen || a.base?.avg || a.avgEntry || a.entryPrice) || null;
    const px0 = +(rc?.baseMid ?? a.base?.mid ?? a.read?.mid ?? a.entryPrice) || null;
    const val0 = +(rc?.sizeThen ?? a.base?.size ?? a.valueUsd) || null;
    const p0 = a.pos0 || {};
    first = { sz: p0.sz ?? (val0 && px0 ? val0 / px0 : null), value: val0, px: px0, avg: avg0, score: rc ? rc.pThen : a.read?.pFollow ?? null };
    if (rc) {
      const pn = rc.posNow || {};
      now = rc.gone ? { gone: true, px: rc.mid, score: rc.pNow }
        : { sz: pn.sz ?? (rc.sizeNow && rc.mid ? rc.sizeNow / rc.mid : null), value: rc.sizeNow, px: rc.mid, avg: rc.avgNow, pnl: rc.whaleNow,
          upnl: pn.upnl, lev: pn.lev, liq: pn.liqPx, score: rc.pNow };
    } else {
      const pnl = avg0 && px0 ? dir * (px0 - avg0) / avg0 : a.read?.moveSinceEntry ?? null;
      now = { ...first, pnl, upnl: p0.upnl, lev: p0.lev, liq: p0.liqPx };
    }
  } else {
    // An add or a trim: the position at the alert (rebuilt from the change) and now.
    const szNow = +a.sizeNow || null;
    const sz0 = a.kind === 'add' && szNow && a.addedSz ? szNow - +a.addedSz : a.kind === 'trim' && szNow && a.remainingPct ? szNow / a.remainingPct : null;
    first = { sz: sz0, value: sz0 && a.entryPrice ? sz0 * a.entryPrice : null, px: +a.entryPrice || null, avg: +a.entryPrice || null };
    now = { sz: szNow, value: +a.valueUsd || null, px: +a.markPx || null, avg: +a.avgEntry || +a.entryPrice || null, pnl: a.whaleRet, upnl: a.upnl };
  }
  const rows = [
    ['Size', szTxt(first.sz), now.gone ? 'closed' : szTxt(now.sz)],
    ['Value', first.value ? money(first.value) : DASH, now.gone ? '$0' : now.value ? money(now.value) : DASH],
    ['Price', first.px ? priceTxt(first.px) : DASH, now.px ? priceTxt(now.px) : DASH],
    ['Avg entry', first.avg ? priceTxt(first.avg) : DASH, now.gone ? DASH : now.avg ? priceTxt(now.avg) : DASH],
    ['Whale P&L', '', now.gone ? 'closed' : sgPct(now.pnl)],
  ];
  if (!now.gone && Number.isFinite(now.upnl)) rows.push(['Whale P&L $', '', `${now.upnl >= 0 ? '+' : '-'}${money(Math.abs(now.upnl))}`]);
  if (first.score != null || now.score != null) rows.push(['Whale-trade score', pc(first.score), pc(now.score)]);
  if (!now.gone && now.lev) rows.push(['Lev · Liq', `${now.lev}x`, now.liq ? priceTxt(now.liq) : 'none']);
  return { header: ['POSITION', 'FIRST', 'NOW'], rows };
}
function recordSection(a) {
  const d30 = a.record?.d30, d7 = a.record?.d7;
  if (!d30 || !d30.closed) return null;
  const on7 = !!(d7 && d7.closed);
  const c7 = (f) => (on7 ? f(d7) : DASH);
  return { header: ['RECORD', '30D', '7D'], rows: [
    ['P&L', money$(d30.pnl || 0), c7((d) => money$(d.pnl || 0))],
    ['Return', pctOf(d30.roi), c7((d) => pctOf(d.roi))],
    ['Win rate', rateOf(d30.winRate), c7((d) => rateOf(d.winRate))],
    ['Closes', intOf(d30.closed), c7((d) => intOf(d.closed))],
    ['Coins', intOf(d30.coins || 0), c7((d) => intOf(d.coins || 0))],
  ] };
}
function exitsSection(ex) {
  if (!ex) return null;
  const pct = (d) => `${d >= 0 ? '+' : '-'}${Math.abs(d * 100).toFixed(1)}%`;
  return { header: ["WHALE'S EXITS", 'PRICE', 'FROM NOW'], rows: [
    ...(ex.tp.length ? ex.tp.slice(0, 2).map((r, i) => [`Take profit${ex.tp.length > 1 ? ` ${i + 1}` : ''}`, priceTxt(r.px), pct(r.dist)]) : [['Take profit', 'none']]),
    ...(ex.sl.length ? ex.sl.slice(0, 2).map((r, i) => [`Stop loss${ex.sl.length > 1 ? ` ${i + 1}` : ''}`, priceTxt(r.px), pct(r.dist)]) : [['Stop loss', 'none']]),
    ...(ex.adds || []).slice(0, 1).map((r) => ['Adds at', priceTxt(r.px), pct(r.dist)]),
  ] };
}
/** The picture's sections for an alert, an add or a trim (exported for the smoke test). */
export const pictureSections = (a, rc = null) =>
  [positionSection(a, rc), recordSection(a), exitsSection(rc ? (rc.gone ? null : rc.exits) : a.exits)].filter(Boolean);
/** { png, caption } for a message, or null when it goes as text. */
export function photoFor(a, text, opts = {}) {
  if (!IMG.available() || a.kind === 'orders') return null;
  const grids = gridsOf(text);
  if (!grids.length) return null;
  const caption = captionFor(a, opts);
  if (!caption || caption.length > CAPTION_MAX) return null;
  const rc = opts.recheck || null;
  const trust = a.record?.trust;
  const badge = { add: 'ADDING', trim: 'TRIM', exit: 'EXIT' }[a.kind] || (trust?.grade ? `GRADE ${trust.grade}` : 'ALERT');
  const name = String(a.trader || '');
  const who = name.length > 34 ? name.slice(0, 33) + '…' : name;
  const opened = a.openedAt ? Date.parse(a.openedAt) : null;
  const when = rc ? `updated ${rc.hhmm} UTC` : a.kind === 'exit' ? `closed ${new Date().toISOString().slice(11, 16)} UTC` : opened ? `opened ${hrsTxt(Date.now() - opened)} ago` : '';
  const sections = a.kind === 'exit' ? IMG.sectionsOf(grids) : pictureSections(a, rc);
  const png = IMG.render({ title: `${String(a.side).toUpperCase()} ${String(a.coin).split(':').pop()}`, subtitle: [who, when].filter(Boolean).join(' · '), badge, sections });
...
async function recheck(q) {
  ...
  if (!a) return answer(`That alert is more than ${ARCHIVE_DAYS} days old, so it can no longer be re-checked.`);
  ...
  a.base.avg ??= +a.avgEntry || +a.entryPrice || null;
  a.base.size ??= a.valueUsd ?? null;
  // The whale's average entry moves when they add; the picture shows it at the alert and now.
  if (!a.kind && !item.gone) {
    const pos = await stillOpen(a).catch(() => null);
    if (pos?.open && Number.isFinite(pos.entryPx) && pos.entryPx > 0) a.avgEntry = pos.entryPx;
    a.posNow = pos?.open ? { sz: Number.isFinite(pos.sz) ? Math.abs(pos.sz) : null, upnl: pos.upnl ?? null, lev: pos.lev ?? null, liqPx: pos.liqPx ?? null } : null;
  }
  a.read = { pFollow: item.pFollow, odds: item.odds, reasons: (item.reasons || []).slice(0, 3), positioning: item.positioning, moveSinceEntry: item.moveSinceEntry, mid: item.mid };
  ...
  const entryRef = +a.avgEntry || +a.entryPrice;
  ...
  const note = [
    ...
    whaleNow == null ? ''
      : `${whaleNow >= 0 ? '🟢' : '🔴'} The whale is ${whaleNow >= 0 ? 'up' : 'down'} ${Math.abs(whaleNow * 100).toFixed(2)}% on this ${a.side.toLowerCase()}, from their ${+a.avgEntry ? 'average entry' : 'entry'} ${px(entryRef)} → ${px(item.mid)}`,
    since == null ? ''
  ...
  const owner = String(q.message.chat.id) === String(cfg.telegram.chatId || '');
  const text = textFor(a, note, owner);
  const nowIso = new Date().toISOString();
  const baseMid = a.base.mid ?? a.entryPrice;
  const rc = { hhmm: nowIso.slice(11, 16), ago, baseMid, mid: item.mid, since, whaleNow,
    whaleThen: a.base.avg && baseMid ? dir * (baseMid - a.base.avg) / a.base.avg : null,
    avgThen: a.base.avg || null, avgNow: item.gone ? null : entryRef || null,
    sizeThen: a.base.size || null, sizeNow: item.gone ? null : a.valueUsd || null,
    pThen: a.base.pFollow ?? null, pNow: item.pFollow ?? null, exits: item.gone ? null : item.exits || null,
    gone: !!item.gone, trimmed: item.trimmed || 0, posNow: item.gone ? null : a.posNow || null,
    driftThen: a.record?.trust?.grade && item.record?.trust?.grade && a.record.trust.grade !== item.record.trust.grade ? a.record.trust : null,
    driftNow: a.record?.trust?.grade && item.record?.trust?.grade && a.record.trust.grade !== item.record.trust.grade ? item.record.trust : null };
  ...
  if (q.message.photo) {
    ...
    const pic = photoFor(a, text, { owner, recheck: rc });
    if (pic) await tgFormImpl('editMessageMedia', { chat_id: q.message.chat.id, message_id: q.message.message_id,
      ...
      caption: captionFor(a, { owner, recheck: rc }).slice(0, CAPTION_MAX), reply_markup: alertButtons(a) }).catch(notModified);
  } else {
  ...
  if (alerts.includes(a)) save('alerts', alerts); else retire(a);   // an archived alert keeps its re-check too
...
async function sendTelegram(a) {
  ...
  if (cfg.telegram.chatId) {
    ...
    sent = await sendCard(cfg.telegram.chatId, ownText, photoFor(a, ownText, { owner: true }), { reply_markup: alertButtons(a) });
...
export async function testAlert() {
  ...
  [a.botEligible, a.botReason] = botEligibility(a);
  alerts.push(a); while (alerts.length > 200) retire(alerts.shift()); save('alerts', alerts);
