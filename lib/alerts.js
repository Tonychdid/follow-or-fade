...
export function publicConfig(canAdmin = true) {
  ...
  return { ...rest, telegram: { connected: !!(telegram.token && telegram.chatId), hasToken: !!telegram.token, botName: canAdmin ? telegram.botName : '', pairCode: canAdmin ? telegram.pairCode || '' : '' },
...
async function watchAlert(a) {
  ...
  if (sz == null || Math.abs(sz) < 1e-12 || Math.sign(sz) !== (a.side === 'Long' ? 1 : -1)) { removeWatch(w); return; }
...
function pushExitAlert(w, extra) {
  ...
  const a = { id: crypto.randomUUID(), t: Date.now(), key: w.key, coin: w.coin, side: w.side, trader: w.trader, address: w.address,
    ...
    specialist: parent?.specialist || null, intelLines: parent?.intelLines || [], read: parent?.read, parentId: w.alertId, ...extra };
...
export async function checkExits() {
  ...
  try {
    for (const w of watches) {
      ...
      if (excluded.isExcluded(w.address)) { w.done = true; continue; }
...
function readLines(a) {
  ...
  if (r.moveSinceEntry != null) {
    ...
    const where = `${a.coin} ${raw >= 0 ? 'up' : 'down'} ${Math.abs(raw).toFixed(2)}% from their entry`;
    out.push(Math.abs(m) < 0.3 ? `Entry now: within ${Math.abs(m).toFixed(2)}% of the whale's price (${where})`
      : m > 0 ? `Entry now: ${where}, so this ${a.side.toLowerCase()} is ${m.toFixed(2)}% up — your entry is ${m.toFixed(2)}% worse than theirs (joining late)`
      : `Entry now: ${where}, so this ${a.side.toLowerCase()} is ${Math.abs(m).toFixed(2)}% underwater — your entry is ${Math.abs(m).toFixed(2)}% better than theirs`);
  }
...
function whoLine(a) {
  ...
  const who = a.specialist ? `${a.coin} specialist` : `Grade ${g || '?'}`;
...
function exitText(a, note = null) {
  ...
  const spec = a.specialist ? ' · SPECIALIST' : '';
  ...
  const profile = `Whale profile: https://app.nansen.ai/profiler?address=${a.address}&chain=hyperliquid`;
  return [...text.filter((l) => l !== profile), ...(note ? ['', note] : []), '', profile].join('\n');
}
...
async function recheck(q) {
  ...
  const note = [
    ...
    since == null ? ''
      ...
      : `${since >= 0 ? '📈' : '📉'} ${a.coin} is ${rawSince >= 0 ? 'up' : 'down'} ${Math.abs(rawSince * 100).toFixed(2)}% since the alert · ${Math.abs(since * 100).toFixed(2)}% ${since >= 0 ? `in this ${a.side.toLowerCase()}'s favour · you'd enter ${Math.abs(since * 100).toFixed(2)}% worse than at the alert` : `against this ${a.side.toLowerCase()} · you'd enter ${Math.abs(since * 100).toFixed(2)}% better than at the alert`}`,
...
export async function verifyTelegram() {
  ...
  const updates = await tg(cfg.telegram.token, 'getUpdates', { offset: cfg.telegram.offset || 0, timeout: 0 });
  const code = String(cfg.telegram.pairCode || '');
  const msg = [...updates].reverse().find((u) => u.message?.chat?.id && (!code || String(u.message.text || '').toUpperCase().includes(code)));
  if (!msg) throw Object.assign(new Error(`No matching message yet. Open Telegram, search @${cfg.telegram.botName}, press Start and send the code ${code}, then try again.`), { status: 409 });
...
const RISK_LINE = '⚠️ If you trade this: 1x leverage and isolated margin. Play money here is unleveraged; perps are not. Not financial advice.';
