...
/**
 * Re-read the whale's resting take profits and stops every few minutes and say when they move.
...
async function checkOrders(w, hlCoin, abs) {
  ...
  if (w.exitsSig == null) { Object.assign(w, { exitsSig: sig, exitsNotified: cur, exitsNotifiedAt: Date.now() }); return; }
...
function exitsBlock(ex) {
  ...
  if (ex.adds.length) out.push(...ex.adds.slice(0, 2).map((r) => `Adds at <code>${esc(priceTxt(r.px))}</code> ${pct(r.dist)}${r.share != null ? ` · ${Math.round(r.share * 100)}% more` : ''}`));
  return out;
}
