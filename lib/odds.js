...
export function reasons(f, ctx) {
  ...
  // Whether a reason counts for or against Follow is read off the LIVE fitted weight, never assumed.
  // The model refits every hour, and a textbook prior ("paying funding is bad") can be the opposite
  // of what the resolved trades say; the reason shown must agree with the price it sits next to.
  const push = (k, text) => {
    const c = (model[k] || 0) * (f?.[k] || 0);   // this feature's pull on the log-odds of Follow
    if (Math.abs(c) >= 0.01) out.push({ w: Math.abs(c), good: c >= 0, text });
  };
  if (ctx.wallet?.closed_trade_count) push('walletEdge', `This wallet won ${pct(ctx.wallet.win_rate)} of ${ctx.wallet.closed_trade_count} closed trades in the 30 days before`);
  if (ctx.sm?.smart_money_volume || ctx.sm?.volume) push('smFlow', f.smFlow >= 0 ? `Smart Money flow on ${ctx.coin} agreed with this trade in the prior 24h` : `Smart Money flow on ${ctx.coin} was against this trade in the prior 24h`);
  if (ctx.crowd?.volume) push('crowdFlow', f.crowdFlow >= 0 ? `The crowd was already piling in the same direction` : `The crowd was on the other side`);
  if (Math.abs(f.funding) > 0.2) push('funding', f.funding > 0 ? `This side was paying funding` : `This side was getting paid funding`);
