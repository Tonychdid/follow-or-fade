...
export async function newRound(playerId) {
  ...
  const ready = deck.filter((h) => !p.played.includes(tradeKey(h.t)));
  ...
  for (let i = 0; i < 10; i++) {
    ...
    return round.pub = {
      ...
      entryPrice: t.price_usd, openedAt: t.block_timestamp, maxHoldHours: MAX_HOLD() / 3600e3, minHoldMinutes: MIN_HOLD() / 60e3,
...
export async function calibrate(limit = Number(process.env.CALIBRATION_SAMPLE || 60)) {
  ...
  save('samples', samples);
  oddsEngine.calibrateLate(liveBets.filter((b) => !b.pick && b.late != null && b.pBase != null && ['won', 'lost', 'cashed'].includes(b.status) && b.whaleRet != null && Math.abs(b.whaleRet) >= 0.0001)
    .map((b) => ({ late: b.late, pBase: b.pBase, followWon: b.whaleRet > 0 })));
  return oddsEngine.calibrate(realSamples());
}
...
export async function buildLiveItem(t) {
  ...
  const pFollow = oddsEngine.lateAdjust(pBase, late);
  const item = {
    ...
    moveSinceEntry, pFollow, odds: oddsEngine.odds(pFollow),
    reasons: [oddsEngine.lateReason(moveSinceEntry, t.side), ...oddsEngine.reasons(f, ctx)].slice(0, 4).map(({ good, text }) => ({ good, text })), positioning,
  ...
  Object.defineProperty(item, 'late', { value: { late, pBase }, enumerable: false });
...
export async function placePickBet(playerId, choice, stake, days = 30) {
  ...
  const entry = await hl.mid(r.pick.coin);
  if (!entry) throw new Error('No live price for ' + r.pick.coin);
...
export async function placeLiveBet(playerId, item, choice, stake, minutes = 60) {
  ...
  const bet = { id: crypto.randomUUID(), playerId, coin: t.coin, whaleSide: t.side, choice, stake,
    ...
    whaleEntry: t.entryPrice, late: t.late?.late ?? null, pBase: t.late?.pBase ?? null, pFollowAtBet: t.pFollow,
      ...
      if (b.ride && !b.demoRide) {
        ...
        if (!e) {
          ...
          if (Date.now() > b.settleAt + 10 * 60e3) {
            const px = (await hl.priceAt(b.coin, b.settleAt).catch(() => null)) ?? (await hl.mid(b.coin).catch(() => null));
            if (px) settleBet(b, px, { whaleClosed: false, whaleHeldMs: b.settleAt - b.placedAt });
          }
          continue;
        }
...
export async function liveBetsFor(playerId) {
  ...
  return Promise.all(mine.map(async (b) => {
    ...
    return { ...b, now, whaleRet, whaleTradeRet, winningNow: Math.abs(whaleRet) < (b.ride || b.pick ? PUSH_BAND() : 0.00003) ? null : winning, band: bandFor(b), cashOut: q ? q.offer : null, pWin: q ? q.pWin : null, myRet: b.choice === 'follow' ? whaleRet : -whaleRet };
  }));
}
