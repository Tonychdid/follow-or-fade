...
function publicPlayer(p) {
  ...
  const { played, inflight, recentWhales: _rw, ...rest } = p;
...
export function leaderboard() {
  return Object.values(players).filter((p) => p.bets >= BOARD_MIN_BETS)
...
export async function newRound(playerId, challengeId = null) {
  ...
  const quick = strict.length ? strict[Math.floor(Math.random() * strict.length)] : null;
...
const isScalper = (d30) => !!(d30 && d30.closed && d30.closed / 30 >= SCALPER_PER_DAY);
...
function trustGrade(d30, d7, coin, addr) {
  ...
  return { grade, score, label, specialist, scalper: isScalper(d30), tradesPerDay: d30.closed ? +(d30.closed / 30).toFixed(1) : null,
    early: !!cls?.early, printer: !!cls?.printer,
    earlyStats: cls?.early ? { goodFinds: cls.goodFinds, finds: cls.finds, capture: cls.medCapture, coins: cls.goodCoins, days: cls.goodDays, best: cls.bestFind } : null,
 ...
 */
function pickFloor(stillIn, slots = FLOOR_SLOTS) {
  const opensByWallet = new Map();
  for (const x of stillIn) opensByWallet.set(x.t.trader_address, (opensByWallet.get(x.t.trader_address) || 0) + 1);
  ...
  const out = [], perCoin = new Map(), usedWallet = new Set();
  let busy = 0;
  ...
  const take = (x) => {
    ...
    const isBusy = cls?.scalper != null ? cls.scalper
      : cls?.pace != null ? cls.pace >= SCALPER_PER_DAY
      : (opensByWallet.get(w) || 0) >= 3;   // never scored: fall back to how often they show up here
...
async function buildFeed() {
  ...
  const slower = byWallet.filter((t) => !pinnedSet.has(t.trader_address) && tclass.classOf(t.trader_address)?.scalper === false).slice(0, 18);
  const slowerSet = new Set(slower.map((t) => t.trader_address));
  const candidates = [...pinned, ...slower,
    ...byWallet.filter((t) => !pinnedSet.has(t.trader_address) && !slowerSet.has(t.trader_address)).slice(0, 26)];
  ...
  const top = [...stillIn.filter((x) => pinnedKeys.has(tradeKey(x.t))),
    ...pickFloor(stillIn.filter((x) => !pinnedKeys.has(tradeKey(x.t))))];
...
async function openLiveBet(p, playerId, t, choice, stake, minutes, ride) {
  ...
  const bet = { id: crypto.randomUUID(), playerId, coin: t.coin, whaleSide: t.side, choice, stake,
    ...
    ...coach.snapshot(t.f, pNow, choice, t.record?.trust?.grade) }; // the probability actually booked, not the card's
  liveBets.push(bet); save('livebets', liveBets); save('players', players); // bet first: a stake is never taken without one
