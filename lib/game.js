...
export async function liveBetsFor(playerId) {
  ...
  return Promise.all(mine.map(async (b) => {
    ...
    return { ...b, now, whaleRet, winningNow: Math.abs(whaleRet) < (b.ride || b.pick ? PUSH_BAND() : 0.00003) ? null : winning, band: bandFor(b), cashOut: q ? q.offer : null, pWin: q ? q.pWin : null, myRet: b.choice === 'follow' ? whaleRet : -whaleRet };
  }));
}
