...
export async function newRound(playerId, challengeId = null) {
  ...
  // A brand-new player was shown a hand on the front door (peekHand reads the first card in the deck).
  // Deal them that exact hand, or the first thing they see is a different trade from the one promised.
  const previewed = deck.find((x) => x.t && x.t.token_symbol);
...
function trustGrade(d30, d7, coin, addr) {
  ...
  const label = mm ? 'Market-maker pattern' : specialist ? `${String(coin).replace(/^\w+:/, '')} specialist` : { 'A+': 'Elite', A: 'Strong hands', B: 'Solid', C: 'Coin flip', D: 'Shaky', F: 'Fade material' }[grade];
  ...
  return { grade, score, label, specialist: mm ? null : specialist, marketMaker: mm, scalper: isScalper(addr),
