...
// ------------------------------------------------------------------ Insider Pick of the Day (Nansen Agent, 24h table)
const PICK_MINUTES = 1440;
export async function pickView(playerId) {
  const r = agent.insiderPick();
  const out = { ...r, desk: agent.status() };
  if (r.pick) {
...
export async function placePickBet(playerId, choice, stake) {
  const p = players[playerId];
  if (!p) throw Object.assign(new Error('Unknown player'), { status: 404 });
  const r = agent.insiderPick();
  ...
  const bet = { id: crypto.randomUUID(), playerId, coin: r.pick.coin, whaleSide: r.pick.side, choice, stake, price: choice === 'follow' ? odds.follow : odds.fade,
    entry, placedAt: Date.now(), settleAt: Date.now() + PICK_MINUTES * 60e3, minutes: PICK_MINUTES, pick: true, pickDay: r.pick.day, trader: 'Nansen Agent', status: 'open' };
