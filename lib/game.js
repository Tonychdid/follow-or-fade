...
export function placeBet(playerId, roundId, choice, stake) {
  ...
  p.played.push(tradeKey(r.trade)); if (p.played.length > 400) p.played.shift();
...
export async function placePickBet(playerId, choice, stake, days = 30) {
  ...
  let entry;
  try { entry = await hl.mid(r.pick.coin); } finally { p.inflight--; }
  if (!entry) { p.bankroll += stake; throw new Error('No live price for ' + r.pick.coin); }
...
export async function placeLiveBet(playerId, item, choice, stake, minutes = 60) {
  ...
  t = t && Date.now() - t.t < 20 * 60e3 ? t.item : null;
  ...
  catch (e) { p.bankroll += stake; throw e; }
  finally { p.inflight--; }
}
...
export function prune() {
  ...
  for (const [id, p] of Object.entries(players)) {
    if (!p.bets && !openBetsOf(id) && Date.now() - (p.createdAt || 0) > 2 * DAY) { delete players[id]; removed++; }
  }
