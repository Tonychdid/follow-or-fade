...
import { gradeV2 } from './gradev2.js';
...
// Whale results inside this band are too small to call: training hands, Ride the Whale and Stock Pick bets push (stake back).
...
// What the odds model is allowed to learn from: everything except the permanent holdout, so a hand
// dealt from the holdout is priced by a model that has never seen it. The proof desk's clean record
// is built on exactly that promise. Falls back to everything only if the split would starve the fit.
const trainingSamples = () => {
  const t = Object.entries(samples).filter(([k, x]) => !x.skip && !proof.isHoldout(k)).map(([, x]) => x);
  return t.length >= 15 ? t : realSamples();
};
oddsEngine.calibrate(trainingSamples()); // start with the odds learned on the last run
...
export function createPlayer(name) {
  ...
  if (Object.keys(players).length >= MAX_PLAYERS) {
    prune({ hard: true });
    if (Object.keys(players).length >= MAX_PLAYERS) throw Object.assign(new Error('The table is full right now, try again in a few minutes.'), { status: 503 });
  }
 ...
 */
const RIGS = new Set((process.env.BOARD_EXCLUDE ?? 'lag1,lag2,lag3,perf,perf2,perf3,smoke,shot,checkpill,sweep,off,tony,simi')
...
async function enrich(t) {
  ...
  // Traced, so the hand can show exactly which Nansen calls priced it (see traced() in nansen.js).
  const { value: [wallet, sm, crowd], calls } = await nansen.traced(() => Promise.all([
  ...
  const ctx = { coin: t.token_symbol, wallet, sm, crowd, sources: calls.map(({ endpoint, how, ms, credits, cohort }) => ({ endpoint, how, ms, credits, cohort })) };
 ...
 *
 * Widening this looks like the obvious way to grow the pool. It is not available: Nansen caps
 * `lookback_hours` on smart-money/perp-trades at 168 and rejects anything larger outright with
 * `value_out_of_range`. That rejection throws out of pool(), which means no deck, no hands dealt and
 * a dead table - so the value is CLAMPED here rather than trusted, and an over-large setting in the
 * environment quietly becomes 168 instead of taking the game down.
 *
 * The pool grows by asking for more pages of the window we are allowed (see wantsNextPage in
 * nansen.js) or by lowering the $25k trade-size floor - not by reaching further back.
 */
const LOOKBACK_MAX = 168; // Nansen's hard ceiling on this endpoint; larger is rejected, not truncated
async function pool() {
  const trades = await nansen.smartMoneyOpens({
    lookbackHours: Math.min(numEnv('POOL_LOOKBACK_HOURS', 168, 1), LOOKBACK_MAX),
  });
...
function roundPub(round) {
  ...
  return {
    ...
    // The call rail: the Nansen requests behind this hand and how far each Nansen-fed number moved the price.
    nansen: { calls: ctx.sources || null, why: oddsEngine.contributions(f) },
...
// The front door shows one deck hand and a new player is dealt exactly that hand. It rotates every
// PREVIEW_ROTATE_MIN minutes, so the site is not greeting every visitor with the same week-old trade
// (the Sep 23 judges were all dealt one ETH long). Picked by a time bucket, not at random, so the
// preview and the first deal agree without the client having to send anything back.
const PREVIEW_ROTATE_MS = numEnv('PREVIEW_ROTATE_MIN', 15, 1) * 60e3;
function previewCard() {
  const ok = deck.filter((x) => x.t && x.t.token_symbol);
  return ok.length ? ok[Math.floor(Date.now() / PREVIEW_ROTATE_MS) % ok.length] : null;
}
...
function botsPlay(t, out, f) {
  try {
    ...
    proof.record({ key: tradeKey(t), coin: t.token_symbol, side: t.side, address: t.trader_address,
      pFollow, pBase: oddsEngine.baseProbability(f), followWon: out.ret > 0, push });
...
export async function newRound(playerId, challengeId = null) {
  ...
  // A brand-new player was shown a hand on the front door (peekHand, the current preview card).
  // Deal them that exact hand, or the first thing they see is a different trade from the one promised.
  const previewed = previewCard();
  const firstHand = !Object.keys(p.played || {}).length && previewed && strict.includes(previewed);
  const quick = firstHand ? previewed : pickFrom.length ? pickFrom[Math.floor(Math.random() * pickFrom.length)] : null;
...
export function placeBet(playerId, roundId, choice, stake) {
  ...
  return {
    ...
    reveal: {
      trader: nansen.whaleName(r.trade.trader_address_label, r.trade.trader_address),
...
// How much raw material the last calibration actually got out of the feed. Published on /api/status
// beside the sample count, because "the pool is not growing" and "the pool is growing fine" look
// identical from outside, and a number nobody can see is a number nobody can check.
let lastPool = { usable: null, fresh: null, taking: null, at: null };
...
export async function calibrate(limit = numEnv('CALIBRATION_SAMPLE', 240, 1)) {
  ...
  const fresh = list.filter((t) => !samples[tradeKey(t)]);
  const todo = fresh.slice(0, limit);
  // Say out loud how much raw material the feed actually handed over. Without this the pool being
  // stuck is invisible: an empty `todo` and a healthy run look identical from the outside, and the
  // difference between "the feed is small" and "we are asking for it wrong" is not guessable.
  console.log(`  Pool: ${list.length} usable trades from the feed, ${fresh.length} not yet sampled, taking ${todo.length}`);
  lastPool = { usable: list.length, fresh: fresh.length, taking: todo.length, at: new Date().toISOString() };
  for (const t of todo) {
  ...
  const out = oddsEngine.calibrate(trainingSamples()); // base model first, never on the holdout
  // Then the level, on hands already dealt and scored: the base-model probability recorded with each
  // (rows from before this existed carry only p, which was the base model then), never the holdout.
  try {
    const dealt = proof.journal(0).filter((r) => !r.push && !r.ho).slice(-600).map((r) => ({ p: r.pb ?? r.p, won: r.won }));
    const rc = oddsEngine.recalibrate(dealt);
    console.log(`  Odds recalibrated on ${rc.n} dealt hands: shift ${rc.offset >= 0 ? '+' : ''}${rc.offset} logit`);
  } catch (e) { console.error('[recal]', e.message); }
...
// Off by default: Nansen counts closes per FILL, so an active directional trader who scales out in
// pieces shows thousands of "closes" and a normal win rate. Only the win-rate rule is on.
const MM_MAX_CLOSED = numEnv('MM_MAX_CLOSED_30D', 1e9, 1);
const MM_MAX_WIN = numEnv('MM_MAX_WIN_RATE', 0.95, 0);
const MM_MIN_SAMPLE = numEnv('MM_MIN_SAMPLE', 200, 1);
// Too fast to copy: a wallet that holds for minutes to a few hours (SCALPER, measured on real round
// trips) AND wins nearly every close. Its edge lives in the minutes a follower spends reading the
// alert, so a copier gets the entry without the exit. Found by the Sep 23 judges: an A+ "Elite"
// wallet winning 90% of 2,154 closes with a 3-hour median hold. Not called a market maker (it does
// hold direction); just not something the table deals as a strong whale or alerts on.
const FAST_MAX_WIN = numEnv('FAST_MAX_WIN_RATE', 0.88, 0);
// Open-loss caps on the live grade (Sep 24 panel, chair G2): uPnL as a share of account value.
const OPEN_LOSS_CAP = numEnv('OPEN_LOSS_CAP_PCT', 0.15, 0);
const OPEN_LOSS_OUT = numEnv('OPEN_LOSS_OUT_PCT', 0.30, 0);
 ...
 */
/**
 * `opts.health` is a live account reading (health.js). When it is not passed, the last cached reading
 * for the wallet is used if there is one, so a card and an alert graded a minute apart agree. It adds
 * two caps, both "if known":
 *   OPEN LOSS  unrealized PnL <= -15% of account value caps the score at 74 (grade B at best);
 *              <= -30% caps it at 35 and the wallet is not alert-eligible (openLossOut).
 *   HOLDS LOSERS  95%+ of 20+ closes won while sitting on deep open losses: capped at 74.
 *              `opts.holdsLosers` is the full verdict (with holding times) when the caller has it;
 *              without it only the account-level half of the rule can be checked here.
 */
function trustGrade(d30, d7, coin, addr, opts = {}) {
  if (!d30 || !d30.closed) return { grade: '?', score: null, label: 'No track record' };
  // No reading passed: the live one if fresh, else the one kept from the last read (roster or alert),
  // so every card is graded with the open-loss caps and the HOLDS LOSERS rule, not only fresh ones.
  const known = opts.health === undefined && addr ? healthLib.latest(addr) : null;
  const health = opts.health !== undefined ? opts.health : known?.health ?? null;
  let hl8 = opts.holdsLosers !== undefined ? opts.holdsLosers : tclass.holdsLosersOf(d30, health);
  if (opts.holdsLosers === undefined && !hl8?.holdsLosers && known?.holds?.holdsLosers) hl8 = known.holds;
  ...
  // Market-maker guard. A wallet winning 95%+ of hundreds of closes is quoting both sides of the
  // book, not calling direction: its "win rate" counts spread captured, and nobody can copy it. Measured on the live floor (Sep 22): one such wallet showed 5,864 wins
  // from 5,864 closes in 7 days and was graded A+. Capped at C, so it is never dealt as a strong
  // whale and never clears the alert bar.
  const mm = (d30.closed || 0) >= MM_MAX_CLOSED || ((d30.winRate ?? 0) >= MM_MAX_WIN && (d30.closed || 0) >= MM_MIN_SAMPLE);
  const tooFast = !mm && isScalper(addr) && (d30.winRate ?? 0) >= FAST_MAX_WIN && (d30.closed || 0) >= MM_MIN_SAMPLE;
  if (mm || tooFast) score = Math.min(score, 55);
  // Live account caps (Sep 24 panel). A record of closed trades says nothing about a book that is
  // deep under water right now, so the open loss caps the grade whatever the closes say.
  const upnlPct = health?.upnlPct ?? null;
  const openLossOut = upnlPct != null && upnlPct <= -OPEN_LOSS_OUT;
  if (openLossOut) score = Math.min(score, 35);
  else if (upnlPct != null && upnlPct <= -OPEN_LOSS_CAP) score = Math.min(score, 74);
  const holdsLosers = !!hl8?.holdsLosers;
  if (holdsLosers) score = Math.min(score, 74);
  ...
  const label = mm ? 'Market-maker pattern' : tooFast ? 'Too fast to copy' : specialist ? `${String(coin).replace(/^\w+:/, '')} specialist` : { 'A+': 'Elite', A: 'Strong hands', B: 'Solid', C: 'Coin flip', D: 'Shaky', F: 'Fade material' }[grade];
  const cls = addr ? tclass.classOf(addr) : null;
  // EARLY is the pattern route only. The second, "proven-early" route (one to three finds on an
  // exceptional record) was removed on Sep 24: 9 of 15 EARLY tags rested on 1-3 finds. The tag still
  // has to be CURRENT, because it waives the size floor: see FIND_MAX_AGE_H in traderclass.js.
  const early = !!cls?.early && tclass.findIsFresh(cls);
  // Grade v2 runs in shadow beside this one and never gates anything (see gradev2.js).
  const v2 = gradeV2({ d30, d7, cls, health, holdsLosers, marketMaker: mm, specialist: mm || tooFast ? null : specialist });
  return { grade, score, label, specialist: mm || tooFast ? null : specialist, marketMaker: mm, tooFast, scalper: isScalper(addr),
    ...
    earlyStats: early ? { goodFinds: cls.goodFinds, finds: cls.finds, capture: cls.medCapture, coins: cls.goodCoins, days: cls.goodDays, best: cls.bestFind,
      via: 'pattern', lastFind: cls.lastGoodFind ?? null, roi: d30.roi ?? null, winRate: d30.winRate ?? null } : null,
    holdsLosers, holdsLosersWhy: hl8?.why || null, openLossPct: upnlPct, openLossOut,
    scoreV2: v2.scoreV2, gradeV2: v2.gradeV2, v2: v2.v2,
...
export async function buildLiveItem(t) {
  ...
  const item = {
    key: tradeKey(t), demo: !!t._demo, coin: t.token_symbol, side: t.side === 'Short' ? 'Short' : 'Long', valueUsd: t.value_usd, trader: nansen.whaleName(t.trader_address_label, t.trader_address),
  ...
  // A grade is a reading of the trailing 7 and 30 days at one moment. The alert froze its grade when it
  // was sent; this card recomputes it now, and for a whale closing a hundred trades a day those two
  // windows can differ by a whole letter. Carry the alert's grade and its time so the card can show
  // both, instead of one surface saying C and the other A with nothing to say which is which.
  try { const ex = exitsOf(item.key); if (ex) item.exits = ex; } catch { /* never block a card */ }
  try {
    const al = alertOf(item.key);
    const g = al?.record?.trust;
    if (g?.grade) item.alertGrade = { grade: g.grade, score: g.score ?? null, at: al.t };
  } catch { /* a missing alert link must never stop a card being dealt */ }
...
let exitsOf = () => null;           // alerts module: the whale's resting TP/SL the watcher last read
export const setExitsLookup = (fn) => (exitsOf = fn);
let rawLookup = () => null;         // alerts module: the trade row behind any alerted position
export const setRawLookup = (fn) => (rawLookup = fn);
export async function refreshLiveItem(key, fallbackTrade = null) {
  ...
  item.gone = !h.open;
  // Correct the headline size to what the whale actually holds. This is why a re-check is worth
  // pressing: the card can have opened with Nansen's single-fill figure and this puts the real
  // position on it.
  if (h.open && Number.isFinite(h.sz) && item.mid > 0) {
    const posValue = Math.abs(h.sz) * item.mid;
    if (posValue > 0) { item.tradeUsd = item.valueUsd; item.valueUsd = posValue; }
  }
  ...
  // A re-check reads the whale's resting take profits and stops fresh (one public Hyperliquid call).
  if (h.open && !t._demo && !nansen.isDemo() && item.mid > 0) {
    const coin = await hl.resolveCoin(t.token_symbol).catch(() => null);
    const orders = coin ? await hl.openOrders(t.trader_address, coin).catch(() => null) : null;
    if (orders) item.exits = exitsLib.classify(orders, item.side, Math.abs(h.sz || 0), item.mid);
  }
  return item;
}

let extraLive = () => [];           // alerts module plugs its pinned trades in here
// The alert a live card came from, if any, so the card can say what grade the alert was sent at.
let alertOf = () => null;
export const setAlertLookup = (fn) => (alertOf = fn);
...
async function holding(t) {
  ...
  // `sz` travels out too. Nansen hands back one fill at a time, so `t.value_usd` is the size of a
  // ROW, not of the whale's position - a split entry arrives looking like a fraction of itself.
  // Anything sizing a copy against that row is sizing against the wrong number.
  return { open, sz, trimmed: open && opened ? Math.max(0, Math.min(1, 1 - Math.abs(sz) / opened)) : 0 };
}
...
// ------------------------------------------------------------------ Stock Pick of the Day (Nansen Agent)
...
export async function placePickBet(playerId, choice, stake, days = 30) {
  ...
  if (!p) throw Object.assign(new Error('Unknown player'), { status: 404 });
  const r = agent.insiderPick();
  if (r.status !== 'ready') throw Object.assign(new Error("Today's Stock Pick isn't ready yet"), { status: 409 });
  ...
  if (liveBets.some((b) => b.playerId === playerId && b.pick && b.pickDay === r.pick.day && ['open', 'cashing'].includes(b.status))) {
    throw Object.assign(new Error("One bet per Stock Pick: you already have today's on the table."), { status: 409 });
  }
  ...
  if (oppositePick) throw (oppositePick.pick
    ? Object.assign(new Error(`You already have a ${oppositePick.choice.toUpperCase()} on today's Stock Pick.`), { status: 409 })
