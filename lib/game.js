...
export function profileOfAddress(addr) {
  ...
  const tagged = c.early || c.printer || c.scalper === true;
  if (!c.grade && !tagged) return null;
  return profileOf({ specialist: false, early: c.early, printer: c.printer, scalper: c.scalper === true, grade: c.grade });
}
...
function botsPlay(t, out, f) {
  try {
    ...
    bots.play({ key: tradeKey(t), pFollow, odds: oddsEngine.odds(pFollow),
      followWon: out.ret > 0, push: Math.abs(out.ret) < PUSH_BAND() });
