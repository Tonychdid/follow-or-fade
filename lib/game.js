...
export async function calibrate(limit = Number(process.env.CALIBRATION_SAMPLE || 60)) {
  ...
  save('samples', samples);
  const settled = liveBets.filter((b) => !b.pick && b.pBase != null && ['won', 'lost', 'cashed'].includes(b.status) && b.whaleRet != null && Math.abs(b.whaleRet) >= 0.0001);
  oddsEngine.calibrateLate(settled.filter((b) => b.late != null).map((b) => ({ late: b.late, pBase: b.pBase, followWon: b.whaleRet > 0 })));
  oddsEngine.calibratePos(settled.filter((b) => b.pos != null).map((b) => ({ pos: b.pos, pBase: b.pBase, followWon: b.whaleRet > 0 })));
...
export async function buildLiveItem(t) {
  ...
  const pFollow = oddsEngine.posAdjust(oddsEngine.lateAdjust(pBase, late), pos);
  const item = {
...
async function openLiveBet(p, playerId, t, choice, stake, minutes, ride) {
  ...
  const bet = { id: crypto.randomUUID(), playerId, coin: t.coin, whaleSide: t.side, choice, stake,
    ...
    whaleEntry: t.entryPrice, late: t.late?.late ?? null, pos: t.late?.pos ?? null, pBase: t.late?.pBase ?? null, pFollowAtBet: t.pFollow,
