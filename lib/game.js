...
async function buildFeed() {
  const trades = excluded.allow(await nansen.smartMoneyOpens({ lookbackHours: 6, pages: 1, perPage: 300, minValueUsd: 10000 }));
...
async function quote(b, now) {
  ...
  // A ride bet ends when the whale closes, not at its 48h backstop, so quoting it against 48h would
...
export async function cashOut(playerId, betId) {
  ...
  p.peak = Math.max(p.peak, p.bankroll);
  maybeBust(p);
  p.history.push({ t: Date.now(), coin: b.coin, side: b.whaleSide, choice: b.choice, stake: b.stake, price: b.price, delta: net, ret: b.whaleRet, bankroll: p.bankroll, live: true, minutes: b.minutes, pick: b.pick, cashed: true, sig: b.sig, pChosen: b.pChosen, modelSide: b.modelSide, grade: b.grade, late: b.late ?? null });
