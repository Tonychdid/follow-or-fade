...
// Human names for the Nansen signals the odds engine uses, by the sign of the signal's VALUE
// ("pos" = the feature is positive for this trade). Which side it favored is a separate question,
// answered by the live model weight in favors(): the words describe the trade, the weight decides
// whether that helped Follow or Fade, so a refit can never make the lesson describe the trade wrongly.
const SIGNALS = {
  walletEdge: { thr: 0.04, pos: "the whale's strong track record", neg: "the whale's weak track record" },
  smFlow: { thr: 0.2, pos: 'Smart Money flow agreeing with the trade', neg: 'Smart Money flow going against the trade' },
  crowdFlow: { thr: 0.2, pos: 'the crowd already piling in (a crowded trade)', neg: 'the crowd sitting on the other side' },
  funding: { thr: 0.3, pos: 'the trade paying funding', neg: 'the trade getting paid funding' },
};
const describe = (k, f) => SIGNALS[k][(f[k] || 0) > 0 ? 'pos' : 'neg'];
...
export function lesson({ f, pFollow, choice, followWon, push, samples }) {
  ...
  const scored = Object.keys(SIGNALS).map((k) => ({ k, desc: describe(k, f), side: favors(k, f), strength: Math.abs((oddsEngine.getModel()[k] || 0) * (f[k] || 0)) }))
  ...
  if (tell) {
    ...
    out.tell = { text: `The tell: ${tell.desc} favored ${tell.side === 'follow' ? 'following' : 'fading'}.`,
  ...
  if (trap) {
    ...
    out.trap = { text: `The trap: ${trap.desc} favored ${trap.side === 'follow' ? 'following' : 'fading'}, and was wrong this time.`,
