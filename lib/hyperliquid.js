...
export async function positionInfo(user, coin) {
  ...
  return p ? { szi: +p.szi, entryPx: +p.entryPx, valueUsd: +p.positionValue, upnl: +p.unrealizedPnl,
    lev: p.leverage?.value != null ? +p.leverage.value : null, liqPx: p.liquidationPx != null ? +p.liquidationPx : null } : { szi: 0 };
}
...
/**
 * A wallet's resting orders on one coin, trigger orders included (take profit, stop loss). Public
 * data: Hyperliquid shows any wallet's open orders. Cached 60 s per wallet and dex, because the
 * floor, the watcher and a re-check can all ask about the same whale within a minute.
 */
const ordersCache = new Map();
export async function openOrders(user, coin) {
  coin = await hlName(coin, true);
  const dex = coin.includes(':') ? coin.split(':')[0] : '';
  const k = user.toLowerCase() + '|' + dex;
  const hit = ordersCache.get(k);
  let rows = hit && Date.now() - hit.t < 60e3 ? hit.rows : null;
  if (!rows) {
    rows = await info(dex ? { type: 'frontendOpenOrders', user, dex } : { type: 'frontendOpenOrders', user });
    if (!Array.isArray(rows)) throw new Error('no open orders answer');
    ordersCache.set(k, { t: Date.now(), rows });
    if (ordersCache.size > 2000) ordersCache.clear();
  }
  return rows.filter((o) => o.coin === coin);
}

/**
 * A wallet's whole perp account on one dex (main when `dex` is ''): account value, total notional
 * and every open position. Free public data. Used by the account health gate in health.js.
 */
export async function clearinghouse(user, dex = '') {
  const st = await info(dex ? { type: 'clearinghouseState', user, dex } : { type: 'clearinghouseState', user });
  if (!st || typeof st !== 'object') throw new Error('no clearinghouse answer');
  return st;
}

/**
 * Account history as Hyperliquid's own portfolio page shows it: for each window ('day', 'week',
 * 'month', 'allTime' and their perp-only twins) a series of [time, accountValue] and [time, pnl].
 * pnlHistory is net of deposits and withdrawals, which is why the health gate reads returns from it.
 */
export async function portfolio(user) {
  const r = await info({ type: 'portfolio', user });
  if (!Array.isArray(r)) throw new Error('no portfolio answer');
  return Object.fromEntries(r.filter((x) => Array.isArray(x) && x.length === 2).map(([k, v]) => [k, v || {}]));
}
