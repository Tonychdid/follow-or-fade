...
async function info(body) {
  ...
  const run = async () => {
    for (let i = 0; i < 6; i++) {
      const res = await fetch(INFO, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(() => null);
  ...
  if (!heavy) return run();
  const p = heavyChain.then(run, run);
...
export async function priceAt(coin, ts) {
  const rows = await candles(coin, ts - 3 * 60e3, ts + 60e3, '1m').catch(() => []);
