...
const MIN_USD = () => Math.max(0, Number(process.env.ROSTER_MIN_USD || 25000));
...
export async function assessPool({ force = false } = {}) {
  ...
  try {
    ...
    for (const w of byWallet.values()) {
      ...
      rec.trust = game.trustGrade(rec.d30, rec.d7, topCoin);
      ...
      whales.push({
        ...
        specialist: !!rec.trust.specialist,
    ...
    console.log(`  Whale roster: ${summary.qualifying}/${summary.walletsSeen} qualify (${summary.scalpers} scalpers) · +${entered.length} in, -${left.length} out`);
