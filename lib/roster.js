...
const LB_TOP = () => Math.max(0, Number(process.env.LEADERBOARD_ALERT_TOP ?? 30));
...
export async function assessPool({ force = false } = {}) {
  ...
  try {
    ...
    // Top performers, recomputed on every sweep, so the list adds and drops with the weekly
    ...
    for (const w of whales) w.leaderboard = topSet.has(w.address);
    ...
    const boosted = top.filter((w) => !w.qualifies).length;
    console.log(`  Whale roster: leaderboard top ${top.length} (min ${LB_MIN_CLOSED()} closes) \u2014 ${boosted} of them are in on the board alone`);
 ...
 * shows. This costs NOTHING - every number here was paid for by the weekly assessment and is sitting
...
export function publicBoard() {
  const rows = (state.whales || [])
    ...
    .map((w) => ({
      address: w.address, trader: w.trader, topCoin: w.topCoin,
      grade: w.grade, score: w.score, inPool: !!w.qualifies, via: w.qualifies || null,
