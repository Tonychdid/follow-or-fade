...
export function publicBoard() {
  const rows = (state.whales || [])
    ...
    .map((w) => {
      // The snapshot is re-assessed weekly; the market-maker guard applies on read so a wallet graded
      ...
      grade: mm ? 'C' : w.grade, score: mm ? Math.min(w.score ?? 55, 55) : w.score, marketMaker: mm,
      inPool: !!w.qualifies && !mm, via: mm ? null : w.qualifies || null,
