...
/**
 * Score one wallet now if the daily roster has not (owner, Sep 24): a whale outside the roster had no
 * hold time at all, so its alert could never be BOT ELIGIBLE. One Hyperliquid read (free), bounded in
 * time: a slow read leaves the wallet unscored, which reads as "hold time unknown", never as a guess.
 */
export async function ensure(addr, { maxAgeH = 24, timeoutMs = 20e3 } = {}) {
  const key = String(addr || '').toLowerCase();
  if (!key) return null;
  const c = map[key];
  if (c && c.at && Date.now() - Date.parse(c.at) < maxAgeH * 3600e3) return c;
  const r = await Promise.race([classify(addr).catch(() => null), new Promise((res) => { const t = setTimeout(() => res(null), timeoutMs); t.unref?.(); })]);
  if (r) {
