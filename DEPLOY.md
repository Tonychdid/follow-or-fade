...
- **`SCAN_MINUTES` (default 2)** is the alert scan interval, applied once to an existing deployment at boot (saved in `alertcfg.json`, changeable afterwards from the settings panel). **`SCAN_MIN_MINUTES` (default 2)** is the hard floor. Each scan costs 5 Nansen credits (about 3,600 a day at 2 minutes), and the feed read is cached for just under one scan so every scan sees fresh trades.
