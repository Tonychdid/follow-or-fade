...
- **`SCAN_MINUTES` (default 10)** is the alert scan interval, applied once to an existing deployment at boot (saved in `alertcfg.json`, changeable afterwards from the settings panel). **`SCAN_MIN_MINUTES` (default 2)** is the hard floor. Each scan costs 5 Nansen credits (about 720 a day at 10 minutes, 3,600 at 2), and the feed read is cached for just under one scan so every scan sees fresh trades.
...
- **Watch your Nansen credits:** open `YOUR_URL/admin/usage.json?token=YOUR_ADMIN_TOKEN` in a browser for the full ledger — calls and credits per endpoint, today's spend, and the Research Desk's remaining budget. Two knobs control the bill: `DAILY_CREDIT_CAP` caps the game's own spend (unset means **no cap**), and `AGENT_DAILY_CREDITS` (default 750) caps the Research Desk separately — the game cap does **not** cover the Agent, and one Insider Pick costs **750 credits**.
