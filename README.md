...
4. **Run it yourself:** `git clone`, then `node server.js`. No install, no build, no API key needed (demo mode). `npm test` runs 90 checks offline.
...
npm test                                      # 90 checks, no key and no network needed
```
