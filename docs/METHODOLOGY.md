# Follow or Fade: how it works, in depth

The [README](../README.md) is the short version. This is the long one: how a hand is built and
settled, every configuration switch, how whales qualify, and the studies behind each rule. Numbers
quoted here are from the day each study was run; the live figures are always at
[`/api/status`](https://follow-or-fade-production.up.railway.app/api/status) and
...
The look is a card table, with sound effects synthesized in the browser (Web Audio, no audio files) and a mute button in the header.
...
|---|---|---|
| `POST /api/v1/smart-money/perp-trades` | 5 | Round pool (7d), Live Floor feed (96h) |
...
| `POST /api/v1/agent/expert` (Nansen Agent) | 750 | **Stock Pick of the Day:** once per UTC day, screens every stock listed on Hyperliquid for insider buying and picks the best setup |
...
| `PUSH_BAND_PCT` | 0.2 | Whale results smaller than ±this % are a push (stake back) on training hands, Ride the Whale and the Stock Pick, and are left out of odds training |
...
| `SCAN_MINUTES` / `SCAN_MIN_MINUTES` | `10` / `2` | How often the alert scan reads the Nansen feed, and the floor. The read is cached for just under one scan, so every scan sees fresh trades. Before Sep 23 the scan ran every 10 minutes with a 15-minute cache, which meant fresh trades only about every 20 minutes (measured on 15 real alerts: a median 13.7 minutes from the whale's first fill to the alert). The cache fix is kept at 10 minutes, so a scan every 10 minutes really reads fresh trades every 10 minutes, for about 720 credits a day |
...
| `ORDERS_CHECK_MIN` / `ORDERS_NOTIFY` / `ORDERS_NOTIFY_MIN` | `3` / off / `10` | For every alerted position still open, the whale's resting orders on that coin (Hyperliquid `frontendOpenOrders`, public) are re-read this often; take profits, stop losses and adds are shown on the alert, the card and every re-check. Follow-up messages when a TP or SL moves are off by default (they were noisy); `ORDERS_NOTIFY=1` turns them back on, at most once per `ORDERS_NOTIFY_MIN`, always reporting the net change. A TP or SL attached to a buy (or sell) limit that has not filled yet belongs to that future add, not to the open position, so it is shown with the add ("adds at 2,324, then TP 2,666") and never as an exit; a trigger whose condition is already true at the current price cannot be live and is set aside |
